...
/**
 * {@link ConnectorPutSenderImpl} クラスの単体テスト。
 */
@RestClientTest(ConnectorPutSenderImpl.class)
public class ConnectorPutSenderImplTest {

    /**
     * テスト対象の {@link ConnectorPutSenderImpl} インスタンス。
     */
    @InjectMocks
    private ConnectorPutSenderImpl connectorPutSender;
    ...
    public void setUp() {
        ...
        ReflectionTestUtils.setField(connectorPutSender, "baseUrl", "http://localhost:8080");
        // Excluded headers setup
        ReflectionTestUtils.setField(connectorPutSender, "excludedHeaders", List.of("X-Forwarded-For", "X-amzn-"));
    }

    /**
     * クエリパラメータを含むsendメソッドのテスト。
     * PUTリクエストが正しいURLに正しいヘッダーで送信され、
     * ボディに正しい値が入ることを検証します。
     */
    @Test
    public void testSend_withquery() {
        ...
        ConnectorPutRequestSendData<String> data = new ConnectorPutRequestSendData("testModel", "{\"key\":\"value\"}", uuid, queryString, "token", headers);
        ...
        // Act
        ConnectorPutRequestReceivedData response = connectorPutSender.send(data);
    ...
    public void testSend_noquery() {
        ...
        headers.add("Link", "http://test.com");
        headers.add("X-Tracking", uuid.toString());
        ConnectorPutRequestSendData<String> data = new ConnectorPutRequestSendData("testModel", "{\"key\":\"value\"}", uuid, null, "token", headers);
