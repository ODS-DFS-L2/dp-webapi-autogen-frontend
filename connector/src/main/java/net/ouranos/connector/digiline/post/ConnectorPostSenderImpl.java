...
@Service
@Slf4j
public class ConnectorPostSenderImpl implements ConnectorPostSender {
    ...
    @Override
    public ConnectorPostRequestReceivedData send(ConnectorPostRequestSendData data) {
        ...
        if (data.queryString() == null) {
            ...
            .toEntity(Object.class);
                ...
                .query(data.queryString()).build().toUri();
                result = this.restClient.build().post()
                .uri(uri)
                .headers(httpHeaders -> {
                    httpHeaders.add("Content-Type", "application/json");
        ...
        return new ConnectorPostRequestReceivedData(data.dataModelName(), result.getBody(), data.tracking(), data.queryString(), result.getStatusCode(), filteredHeaders);
    }
}
